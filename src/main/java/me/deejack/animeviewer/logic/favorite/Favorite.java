...
public final class Favorite {
  ...
  private final Set<AnimeImpl> favorites = new HashSet<>();
  private final AnimeSerializer serializer = new AnimeSerializer<AnimeImpl>(AnimeImpl.class);
  ...
  public void addFavorite(AnimeImpl element) {
    favorites.add(element);
  }

  public void removeFavorite(AnimeImpl element) {
    favorites.remove(element);
  }

  public Set<AnimeImpl> getFavorites() {
  ...
  public boolean loadFromFile(File input) throws IOException {
    ...
    List<AnimeImpl> elements = serializer.deserialize(json);

    favorites.addAll(elements);
    return true;
  }

  public boolean contains(AnimeImpl anime) {
    for (AnimeImpl favorite : favorites)
      if (favorite.getAnimeInformation().getUrl().equals(anime.getAnimeInformation().getUrl()))
        return true;
    return false;
  }

  public AnimeImpl get(String url) {
    for (AnimeImpl favorite : favorites)
